...
function kunlunFeature(item) { const title = item.title || ''; return { type: 'Feature', geometry: { type: 'Point', coordinates: [90.0, 35.0] }, properties: { title, original_title: item.original_title, source: item.source || '', link: item.link || '', published_at: item.published_at || null, representative_place: '崑崙山（未解決仮置き）', place_label: '中国-崑崙山-未解決仮置き', sort_place_label: '中国-崑崙山-未解決仮置き', geocode_query: 'Kunlun Mountains, China', place_type: 'unresolved_placeholder', location_precision: 'placeholder', place_reason: '場所を特定できなかったため、未解決ニュースの仮置き地点として崑崙山に置いています。', confidence: 0, rule_id: 'unresolved_kunlun_placeholder' } }; }
function kunlunFeatures(unresolvedItems) { return (unresolvedItems || []).filter(item => item && item.title).map(kunlunFeature); }
...
function addStats(stats, createdAt) { const block = document.createElement('div'); block.className = 'stats'; block.innerHTML = `<div class="stat"><span class="stat-value">${esc(stats.collected ?? '-')}</span><span class="stat-label">取得</span></div><div class="stat"><span class="stat-value">${esc(stats.topics ?? '-')}</span><span class="stat-label">表示</span></div><div class="stat"><span class="stat-value">${esc(stats.unresolved ?? '-')}</span><span class="stat-label">崑崙山</span></div>`; statusEl.after(block); block.after(ensureCountryIndicator()); setUpdatedAt(createdAt); updateVisibleCountry(); }
